...
import MoviesToWatchPage from "./movies-to-watch";
...
describe("Movie to watch Page", () => {
  ...
  describe("Movies to watch List", () => {
    it("should render message when there are no movies in movies to watch list", () => {
      // When
      const component = mount(
        <Provider store={store}>
          <BrowserRouter>
      ...
      // Then
      expect(component.find(".movies-to-watch--no-movies").length).toEqual(1);
      expect(component.find("MovieSuggestion").length).toEqual(0);
    });

    it("should render movies when there are movies in movies to watch list", () => {
      // Given
      const store = mockStore({
        moviesToWatch: [MOCK_MOVIE_TO_WATCH],
      });
      ...
      // Then
      expect(component.find(".movies-to-watch--no-movies").length).toEqual(0);
      expect(component.find("MovieSuggestion").length).toEqual(1);
      expect(component.find("MovieSuggestion").props().movie).toEqual(
        MOCK_MOVIE_TO_WATCH
      );
    });
  });
  ...
  describe("Delete Modal", () => {
    ...
    it("should render the delete modal when user delete a movie", () => {
      ...
      component.find("MovieSuggestion").props().onToggleDeleteModal();
      component.update();
